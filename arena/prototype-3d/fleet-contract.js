export function fleetHull(manifest,id){
 ...
 if(!Object.hasOwn(h,'code')||!(typeof h.code==='string'&&/^[A-Z]{3}-\d{2}(?:\d{2})?$/.test(h.code)))throw Error('Missing class-code decision: '+id);
 return h;
}
export function validateFleetManifest(m){
 if(m.format!=='tabletop-complete-library/1'||m.hulls.length!==25||new Set(m.hulls.map(h=>h.id)).size!==25||new Set(m.hulls.map(h=>h.source)).size!==25)throw Error('Incomplete fleet contract');
