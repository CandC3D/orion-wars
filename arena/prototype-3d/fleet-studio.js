...
try{
 ...
 let assembly=null,hull=null,current=null,map=null,record=null,camera=perspective,post=null,base=null,rim=null;
 function dispose(){if(!assembly)return;scene.remove(assembly);const disposed=new Set();assembly.traverse(o=>{if(!o.isMesh)return;o.geometry.dispose();if(o.material===black||disposed.has(o.material))return;disposed.add(o.material);o.material.userData.paintEdges?.dispose();if(o.material.map)o.material.map.dispose();o.material.dispose();});}
 async function load(id){
  ...
  const palette=Object.keys(record.regions).map(k=>factionRegion(current.faction,k));map=validateRegionMap(regions,current.faction,{palette,confirmedEffects:[]});
  ...
  dispose();assembly=new THREE.Group();scene.add(assembly);hull=physicalMesh(current.name,paintGeometry,material);assembly.add(hull);
  hull.updateMatrixWorld(true);const ray=new THREE.Raycaster(new THREE.Vector3(0,-30,0),new THREE.Vector3(0,1,0)),hit=ray.intersectObject(hull,false)[0];if(!hit)throw Error('No central underside attachment on '+id);
  hull.position.y=BOARD.top+BASE_APEX+mm(SIZES.postHeight)-hit.point.y;
  base=physicalMesh('black FASA pyramid',baseGeometry(),black);post=physicalMesh('black tapered 30 mm post',postGeometry(),black);base.position.y=BOARD.top;post.position.y=BOARD.top+BASE_APEX+mm(SIZES.postHeight/2);assembly.add(base,post);
  rim=classRim(current.code);if(rim){rim.position.y=BOARD.top;assembly.add(rim);}
 ...
 function measurements(){
  const p=new THREE.Box3().setFromObject(post),b=new THREE.Box3().setFromObject(base),ray=new THREE.Raycaster(new THREE.Vector3(0,p.min.y-.01,0),new THREE.Vector3(0,1,0)),hit=ray.intersectObject(hull,false)[0];
 ...
 function blackout(){const lights=[];scene.traverse(o=>{if(o.isLight){lights.push([o,o.intensity]);o.intensity=0;}});hull.material.envMapIntensity=0;const bg=scene.background;scene.background=new THREE.Color(0);render();const gl=renderer.getContext(),px=new Uint8Array(1000*720*4);gl.readPixels(0,0,1000,720,gl.RGBA,gl.UNSIGNED_BYTE,px);let count=0;for(let i=0;i<px.length;i+=4)if(px[i]||px[i+1]||px[i+2])count++;for(const [l,v] of lights)l.intensity=v;hull.material.envMapIntensity=.6;scene.background=bg;render();return count;}
 window.fleetStudio=Object.freeze({ready:true,load,capture,blackout});status.textContent='Ready';
